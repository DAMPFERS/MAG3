...
# Поиск моды
mode_value <- as.numeric(names(which.max(table(data$качество.документирования))))
print(mode_value)
...
# для признака "процент выполнения разработок в срок"
...
variable_name <- "X..выполнения.разработок.в.срок..в.рамках.бюджета..с.требуемым.функционалом"

# Проверка нормальности для группы 1
cat("=== ПРОВЕРКА НОРМАЛЬНОСТИ ДЛЯ ГРУППЫ 1 (стаж < 5 лет) ===\n")
data_group1 <- group1[[variable_name]]

# Критерий Шапиро-Уилка
shapiro_test_group1 <- shapiro.test(data_group1)
cat("Критерий Шапиро-Уилка:\n")
cat("W =", round(shapiro_test_group1$statistic, 4), 
    "p-value =", format.pval(shapiro_test_group1$p.value, digits = 4), "\n")

# Критерий Крамера-Мизеса
cramer_test_group1 <- cvm.test(data_group1)
cat("\nКритерий Крамера-Мизеса:\n")
cat("W =", round(cramer_test_group1$statistic, 4), 
    "p-value =", format.pval(cramer_test_group1$p.value, digits = 4), "\n")

# Критерий Андерсона-Дарлинга
anderson_test_group1 <- ad.test(data_group1)
cat("\nКритерий Андерсона-Дарлинга:\n")
cat("A =", round(anderson_test_group1$statistic, 4), 
    "p-value =", format.pval(anderson_test_group1$p.value, digits = 4), "\n")

# Визуальная проверка с помощью QQ-plot
par(mfrow = c(1, 2))
qqnorm(data_group1, main = "QQ-plot для группы 1\n(стаж < 5 лет)")
qqline(data_group1, col = "red")

# Проверка нормальности для группы 2
cat("\n\n=== ПРОВЕРКА НОРМАЛЬНОСТИ ДЛЯ ГРУППЫ 2 (стаж > 5 лет) ===\n")
data_group2 <- group2[[variable_name]]

# Критерий Шапиро-Уилка
shapiro_test_group2 <- shapiro.test(data_group2)
cat("Критерий Шапиро-Уилка:\n")
cat("W =", round(shapiro_test_group2$statistic, 4), 
    "p-value =", format.pval(shapiro_test_group2$p.value, digits = 4), "\n")

# Критерий Крамера-Мизеса
cramer_test_group2 <- cvm.test(data_group2)
cat("\nКритерий Крамера-Мизеса:\n")
cat("W =", round(cramer_test_group2$statistic, 4), 
    "p-value =", format.pval(cramer_test_group2$p.value, digits = 4), "\n")

# Критерий Андерсона-Дарлинга
anderson_test_group2 <- ad.test(data_group2)
cat("\nКритерий Андерсона-Дарлинга:\n")
cat("A =", round(anderson_test_group2$statistic, 4), 
    "p-value =", format.pval(anderson_test_group2$p.value, digits = 4), "\n")

# Визуальная проверка с помощью QQ-plot
qqnorm(data_group2, main = "QQ-plot для группы 2\n(стаж > 5 лет)")
qqline(data_group2, col = "red")
...
if (shapiro_test_group1$p.value > 0.05) {
  cat("• Для группы 1 (стаж < 5 лет): распределение процента выполнения разработок в срок НЕ ОТКЛОНЯЕТСЯ от нормального\n")
} else {
  cat("• Для группы 1 (стаж < 5 лет): распределение процента выполнения разработок в срок ОТКЛОНЯЕТСЯ от нормального\n")
}

if (shapiro_test_group2$p.value > 0.05) {
  cat("• Для группы 2 (стаж > 5 лет): распределение процента выполнения разработок в срок НЕ ОТКЛОНЯЕТСЯ от нормального\n")
} else {
  cat("• Для группы 2 (стаж > 5 лет): распределение процента выполнения разработок в срок ОТКЛОНЯЕТСЯ от нормального\n")
}

# Интерпретация в терминах прикладной задачи
cat("\n=== ИНТЕРПРЕТАЦИЯ В ТЕРМИНАХ ПРИКЛАДНОЙ ЗАДАЧИ ===\n")
cat("Проверка нормальности распределения позволяет определить:\n")
cat("1. Возможность применения параметрических статистических методов\n")
cat("2. Характер вариативности показателей эффективности программистов\n")
cat("3. Наличие систематических факторов, влияющих на результативность работы\n\n")

if (shapiro_test_group1$p.value > 0.05 & shapiro_test_group2$p.value > 0.05) {
  cat("ОБА распределения соответствуют нормальному закону, что свидетельствует о:\n")
  cat("• Естественной, случайной вариативности показателей в обеих группах\n")
  cat("• Возможности применения t-критериев и ANOVA для сравнения групп\n")
  cat("• Отсутствии выраженных аномалий в распределении эффективности\n")
} else if (shapiro_test_group1$p.value <= 0.05 & shapiro_test_group2$p.value > 0.05) {
  cat("Распределение в группе 1 отклоняется от нормального, в группе 2 - нормальное:\n")
  cat("• В группе junior-программистов (стаж < 5 лет) эффективность распределена неравномерно\n")
  cat("• Возможно наличие подгрупп с разным уровнем подготовки или мотивации\n")
  cat("• В группе senior-программистов эффективность распределена более предсказуемо\n")
} else if (shapiro_test_group1$p.value > 0.05 & shapiro_test_group2$p.value <= 0.05) {
  cat("Распределение в группе 1 нормальное, в группе 2 отклоняется от нормального:\n")
  cat("• Junior-программисты демонстрируют более стабильные показатели\n")
  cat("• В группе senior-программистов возможна поляризация результатов\n")
  cat("• Senior-специалисты могут разделяться на высокоэффективных и средних\n")
} else {
  cat("ОБА распределения отклоняются от нормального:\n")
  cat("• Эффективность программистов в обеих группах имеет неслучайный характер распределения\n")
  cat("• Существуют систематические факторы, влияющие на результативность работы\n")
  cat("• Рекомендуется использование непараметрических статистических методов\n")
}
...
# Проверка связи между качественной переменной "степень удовлетворенности заказчика" (качественная оценка)
# и количественной переменной "процент разработок, выполненных в срок" с помощью критерия Краскела-Уоллиса

# Преобразуем качественную оценку в фактор, если это еще не сделано
data$степень.удовлетворенности.заказчика..качественная.оценка. <- as.factor(data$степень.удовлетворенности.заказчика..качественная.оценка.)

# Проведение теста Краскела-Уоллиса
kruskal_test_result <- kruskal.test(X..выполнения.разработок.в.срок..в.рамках.бюджета..с.требуемым.функционалом ~ степень.удовлетворенности.заказчика..качественная.оценка., data = data)

# Вывод результатов
print("Результаты критерия Краскела-Уоллиса:")
...
# 5.3 Рассчет коэффициентов корреляции, 5.5
...
names(M1)
print(pcor(c(3, 5, 1, 2, 4), cov(M1)))
print(pcor(c(3, 5, 1, 2, 4), cov(M2)))
...
# 
# Создаем подвыборку только с количественными переменными
numeric_data <- data[, sapply(data, is.numeric)]


# Матричный график с помощью ggpairs
# Построение комплексного матричного графика
ggpairs(numeric_data) +
  ggtitle("Матричный график количественных переменных")

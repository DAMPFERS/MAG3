...
class RightCornerTaskSolution(TaskSolution):
    ...
    def solve(self):
        ...
        while True:
            ...
            # Если робот на перекрестке, поворачиваем направо
            if onCrossRoad:
                # Поворачиваем направо
                linear_velocity = 0.2  # Уменьшаем скорость для более плавного поворота
                angular_velocity = -0.5  # Угловая скорость для поворота направо
                for i in range(67):  # Поворачиваем направо за 67 шагов
                    obs, _, _, _ = env.step([linear_velocity, angular_velocity])
                    env.render()

                # Продолжаем движение прямо после поворота
                linear_velocity = 1.0
                angular_velocity = 0.0
                for i in range(4):  # Двигаемся прямо за 4 шага
                    obs, _, _, _ = env.step([linear_velocity, angular_velocity])
                    env.render()

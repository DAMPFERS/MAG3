...
def main():
    ...
    angle_step = 0.5    # Шаг сканирования, в градусах
    ...
    grid, origin, res = occupancy_grid.buiildOccupancyGrid(pts1_world, resolution=0.1, padding=0.5)
    ...
    # print(f"dx:{found_pose2_bf[0] - pose2[0]}, dy{found_pose2_bf[1] - pose2[1]}, dt{found_pose2_bf[2] - pose2[2]}")
    ...
    print(f"Точность определения положения (%): {trans_error_bf / sqrt(w*w + h*h) * 100:.2f}% для bf, {trans_error_icp / sqrt(w*w + h*h) * 100:.2f}% для ICP")
    
    # Отрисовка: комната, оба скана и карты
    plt.figure(figsize=(5,3))
    plt.subplot(1, 1, 1)                # (строки, стоблцы, индекс)
    ...
    # plt.imshow(grid.T, origin="lower", extent=(  # Транспонируем для imshow
    #     origin[0], origin[0] + grid.shape[0] * res,  # x: nx * resolution
    #     origin[1], origin[1] + grid.shape[1] * res   # y: ny * resolution
    # ))
